...
const isSudokuItemValid = (item: string): boolean => (
  item === EMPTY_VALUE ||
  (
    Number(item) >= Number(MIN_NUMBER)
    &&
    Number(item) <= Number(MAX_NUMBER)
  )
);
...
const isSudokuBoardSize = (board: string[][] = []): boolean => {
  return board.length === BOARD_DIMENSION && board.every((row: string[]) => row.length === BOARD_DIMENSION)
}
...
const getBoxByIndex = (board: string[][] = [], sectorIndex: number): string[] => {
  ...
  const getBoxStartingCoordinates = (row: number, column: number): number[] => ([row * BOX_SIZE, column]);


  const buildSectorBox = (board: string[][] = [], startingCoordinates: number[] = []): string[] => {
    const box: string[] = [];
  ...
  return buildSectorBox(board, getBoxStartingCoordinates(defineStartingRow(sectorIndex), defineStartingColumn(sectorIndex)));
}
...
const hasDuplicates = (arr: string[] = []): boolean => {
  return arr.some((item: string, index: number, arr: string[]) => index !== arr.indexOf(item))
}
